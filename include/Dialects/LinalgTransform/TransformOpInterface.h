...
public:
  ...
  protected:
    ...
    /// handle changed. Concrete extensions that are willing not be notified
    ...
    void updatePayloadOps(Value handle,
      ...
      return state.updatePayloadOps(handle, callback);
    }
  ...
  std::enable_if_t<std::is_base_of<Extension, Ty>::value, Ty &>
  addExtension(Args &&...args) {
  ...
  std::enable_if_t<std::is_base_of<Extension, Ty>::value, Ty &> getExtension() {
  ...
  template <typename Ty>
  std::enable_if_t<std::is_base_of<Extension, Ty>::value> removeExtension() {
